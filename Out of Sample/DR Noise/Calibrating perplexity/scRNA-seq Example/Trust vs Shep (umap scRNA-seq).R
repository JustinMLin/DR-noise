...
df = data.frame(neighbors = rep(neighbors, each = b),
                trust = trusts,
                shep = sheps,
                trust_noise = trusts_noise,
                shep_noise = sheps_noise)

p1 = ggplot(df, aes(x = trust, y = shep, col = neighbors)) + 
  geom_point(size = 2) + 
  scale_color_viridis() + 
  labs(x = "Trustworthiness", y = "Shepard Goodness", title = "Replicating Signal")


p2 = ggplot(df, aes(x = trust_noise, y = shep_noise, col = neighbors)) + 
  geom_point(size = 2) + 
  scale_color_viridis() + 
...
save(Y, df, p1, p2, file = "~/DR noise/Output/trust vs shep (umap scRNA-seq).Rda")

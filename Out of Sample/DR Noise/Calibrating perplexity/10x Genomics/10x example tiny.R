...
for (i in 1:length(neighbors)) {
  for (j in 1:b) {
    ...
    sample_indices = sample(1:n, 400)
    
    trust = trustworthiness_full_approx(Y, X_umap, k, sample_indices)
    trust_noise = trustworthiness_full_approx(Z, X_umap, k, sample_indices)
...
save(Y, df, best_X, best_n_neighbors, best_trust, best_X_noise, best_n_neighbors_noise, best_trust_noise, file = "~/DR noise/Output/10x example tiny.Rda")
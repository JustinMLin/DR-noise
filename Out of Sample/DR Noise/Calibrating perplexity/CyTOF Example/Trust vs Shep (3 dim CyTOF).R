...
par = PARALLEL(data)
barplot(par$eigenvalues_PCA[,"Real Eigenvalues"])

as.data.frame(par$eigenvalues_PCA) %>%
  janitor::clean_names() %>%
  mutate(x = 1:length(means)) %>%
  ggplot(aes(x = x, y = real_eigenvalues)) +
    geom_col() +
    labs(y = "Eigenvalue", title = "Parallel Analysis Eigenvalues") + 
...
# save(Y, df, p1, p2, file = trust vs shep (3 dim CyTOF).Rda")
save(Y, df, p1, p2, file = "~/DR noise/Output/trust vs shep (3 dim CyTOF).Rda")

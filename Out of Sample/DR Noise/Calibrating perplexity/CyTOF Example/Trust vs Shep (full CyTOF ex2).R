...
# data = read.delim("../../../../../CyTOF data/Exp9_NK_uns_CD4_inf.txt")
data = read.delim("Data/Exp9_NK_uns_CD4_inf.txt")
data = data %>%
  select(-c(1,2)) %>%
  sample_n(5000) %>%
...
# save(Y, df, p1, p2, file = trust vs shep (reduced CyTOF ex2).Rda")
save(Y, df, p1, p2, file = "~/DR noise/Output/trust vs shep (full CyTOF ex2).Rda")

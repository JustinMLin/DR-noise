...
data = read.delim("Data/Exp7_NK_uns_CD4_stim.txt")
data = data %>%
  select(-c(1,2)) %>%
  sample_n(5000) %>%
...
# save(Y, df, p1, p2, file = trust vs shep (reduced CyTOF ex3).Rda")
save(Y, df, p1, p2, file = "~/DR noise/Output/trust vs shep (full CyTOF ex3).Rda")
